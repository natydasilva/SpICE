...
plot_alpha <-
  function(DD1,
    ...
    if (is.null(resp)) {
      id.smp <-
        DD1 |> dplyr::mutate(rr = 1:nrow(DD1)) |> dplyr::sample_frac(nfrac) |> dplyr::pull(rr)
    } else {
      qs = stats::quantile(resp, probs = seq(0, 1, .1))
      id.smp <-
        DD1 |> dplyr::mutate(qq = cut(resp, breaks = qs, include.lowest = TRUE),
                             rr = 1:nrow(DD1)) |>
    ...
    DD1 <- DD1 |> dplyr::arrange({
      {
        yhat
      }
    })
    zz <- DD0 |>  split(f = dplyr::pull(DD0, {
      {
        yhat
      }
    }))
    ...
    D0.smp <-
      sqrt(stats::dist(ice.smooth[id.smp, ]) ^ 2 + stats::dist(ice.deriv[id.smp, ]) ^
             2)
    #-------------
    ...
    alpha.info <- NULL
    for (kk in clRange) {
      alp.opt <-   ClustGeo::choicealpha(
        D0 = D0.smp,
        D1 = D1.smp,
        range.alpha = alpRange,
        K = kk,
        scale = TRUE,
        graph = FALSE
      )
